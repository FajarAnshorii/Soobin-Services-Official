...
export default function KalkulatorIPKPage() {
  ...
  // University Max Scale (Manual input)
  ...
  // Simulator Inputs for Mode 2
  const [currentIpk, setCurrentIpk] = useState<string>('3.35');
  const [completedSks, setCompletedSks] = useState<string>('110');
  const [targetIpk, setTargetIpk] = useState<string>('3.51');
  const [remainingSks, setRemainingSks] = useState<string>('34');
  ...
  const calculationResult = useMemo(() => {
    ...
    // Predikat Kelulusan (Standard Dikti based on 4.0 scale normalized)
    const ratio = boundedGpa / maxScale;
    let predicate = 'Memuaskan';
    let predicateColor = 'text-slate-800 bg-slate-100 border-slate-200';

    if (ratio >= 0.875) {
      // e.g. 3.51 - 4.00 on 4.0 scale
      predicate = 'Dengan Pujian (Cumlaude)';
      predicateColor = 'text-emerald-700 bg-emerald-50 border-emerald-200';
    } else if (ratio >= 0.75) {
      // e.g. 3.00 - 3.50 on 4.0 scale
      ...
      predicateColor = 'text-red-700 bg-red-50 border-red-200';
    }
  ...
  const simulationResult = useMemo(() => {
    const curIpk = parseFloat(currentIpk);
    const compSks = parseFloat(completedSks);
    const tarIpk = parseFloat(targetIpk);
    const remSks = parseFloat(remainingSks);
    ...
    if (
      ...
      return {
        ...
        isAchievable: false,
        totalSksAccum: 0,
        message: 'Silakan lengkapi parameter simulasi di atas.',
      };
    }

    const totalSksAccum = compSks + remSks;
    const targetTotalScore = tarIpk * totalSksAccum;
    const currentTotalScore = curIpk * compSks;
    const requiredScore = targetTotalScore - currentTotalScore;
    const requiredIps = requiredScore / remSks;

    const isAchievable = requiredIps <= maxScale && requiredIps >= 0;

    let message = '';
    if (requiredIps > maxScale) {
      message = `Target membutuhkan rata-rata IPS ${requiredIps.toFixed(2)}, melebihi batas maksimal skala (${maxScale.toFixed(2)}). Anda perlu mengambil Semester Pendek (SP) untuk memperbaiki nilai matkul lama atau menambah SKS.`;
    } else if (requiredIps <= 0) {
      message = `Target IPK Anda (${tarIpk}) sudah otomatis tercapai bahkan jika Anda mendapatkan nilai minimal di sisa semester!`;
    } else {
      message = `Target SANGAT REALISTIS! Anda wajib mempertahankan rata-rata IPS minimal ${requiredIps.toFixed(2)} di sisa ${remSks} SKS untuk lulus dengan IPK ${tarIpk.toFixed(2)}.`;
    }
  ...
  }, [currentIpk, completedSks, targetIpk, remainingSks, maxScale]);
    ...
    <main className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <section className="pt-28 sm:pt-36 pb-8 bg-white border-b border-slate-200">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-slate-100 border border-slate-300 text-slate-700 text-xs font-bold uppercase tracking-wider mb-3">
            ...
            <span>Mini Tool Akademik & Kelulusan</span>
          </div>
          <h1 className="text-2xl sm:text-4xl lg:text-5xl font-extrabold text-slate-900 tracking-tight">
            Kalkulator IPK & Simulasi Target Cumlaude
          </h1>
          <p className="mt-3 text-xs sm:text-base text-slate-600 max-w-3xl mx-auto leading-relaxed">
            Hitung Indeks Prestasi Semester (IPS), IPK kumulatif, serta simulasikan target nilai sisa semester untuk meraih predikat <i>Cumlaude</i> sesuai skala dan sistem penilaian kampus Anda.
          </p>
          ...
          <div className="mt-8 inline-flex p-1.5 bg-slate-100 border border-slate-300/80 rounded-2xl max-w-md w-full">
            ...
            >
              ...
              <span>Kalkulator IPK Semester</span>
            </button>
            <button
              type="button"
            ...
            >
              ...
              <span>Simulasi Target Cumlaude</span>
            </button>
          </div>
        </div>
      </section>
      ...
      <section className="py-8 sm:py-12 flex-1">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
                ...
                <div className="flex items-center justify-between pb-3 border-b border-slate-200 mb-4">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="px-2.5 py-1 text-[11px] font-bold text-slate-500 hover:text-red-600 rounded cursor-pointer"
                    >
                      Reset
                    </button>
                    <button
          ...
          {/* TAB 2: SIMULATOR TARGET CUMLAUDE */}
          {activeTab === 'simulator' && (
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
              
              {/* Left Column: Target Inputs (5 Cols) */}
              <div className="lg:col-span-5 bg-white border border-slate-200 rounded-2xl p-5 sm:p-6 shadow-sm space-y-4">
                <div className="flex items-center gap-2 pb-3 border-b border-slate-200">
                  ...
                  <div>
                    <h3 className="text-xs font-bold text-slate-900">Parameter Target Kelulusan</h3>
                    <p className="text-[10px] text-slate-500">Hitung target nilai minimal di sisa semester.</p>
                  </div>
                </div>
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-800 mb-1">
                    IPK Kumulatif Saat Ini <span className="text-red-500">*</span>
                  </label>
                  <input
                    type="number"
                    step="0.01"
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-800 mb-1">
                    Total SKS yang Sudah Lulus <span className="text-red-500">*</span>
                  </label>
                  <input
                    type="number"
                    min="1"
                    ...
                    className="w-full h-11 px-3.5 bg-slate-50 border border-slate-300 focus:border-slate-800 rounded-xl text-xs font-bold text-slate-900 outline-none"
                  />
                  <p className="text-[10px] text-slate-500 mt-1">SKS yang sudah memiliki nilai resmi di transkrip.</p>
                </div>

                {/* Target IPK Kelulusan */}
                <div>
                  <label className="block text-xs font-bold text-slate-800 mb-1">
                    Target IPK Kelulusan yang Diinginkan <span className="text-red-500">*</span>
                  </label>
                  <div className="flex gap-2 mb-2">
                    {[
                      { label: 'Cumlaude (3.51)', val: '3.51' },
                      { label: '3.75', val: '3.75' },
                      { label: '3.80', val: '3.80' },
                    ].map((item) => (
                      <button
                ...
                {/* Sisa SKS yang Akan Diambil */}
                <div>
                  <label className="block text-xs font-bold text-slate-800 mb-1">
                    Sisa SKS yang Akan Diambil <span className="text-red-500">*</span>
                  </label>
                  <input
                    type="number"
                    min="1"
                    value={remainingSks}
                    onChange={(e) => setRemainingSks(e.target.value)}
                    placeholder="Contoh: 34"
                    className="w-full h-11 px-3.5 bg-slate-50 border border-slate-300 focus:border-slate-800 rounded-xl text-xs font-bold text-slate-900 outline-none"
                  />
                  <p className="text-[10px] text-slate-500 mt-1">Termasuk SKS skripsi, KKN, magang, dan sisa mata kuliah.</p>
                </div>
              </div>
              ...
              <div className="lg:col-span-7 space-y-6">
                ...
                <div className="bg-white border border-slate-200 rounded-2xl p-5 sm:p-6 shadow-sm">
                  <div className="flex items-center gap-2 pb-3 border-b border-slate-200 mb-4">
                    ...
                    <div>
                      <h3 className="text-xs font-bold text-slate-900">Hasil Analisis Simulasi</h3>
                      <p className="text-[10px] text-slate-500">Estimasi nilai yang wajib diraih di semester tersisa.</p>
                    </div>
                  </div>
                  ...
                  {simulationResult.isValid ? (
                    <div className="space-y-4">
                      <div className={`p-5 rounded-2xl border text-center ${
                        simulationResult.isAchievable
                          ? 'bg-emerald-950 text-white border-emerald-800'
                          : 'bg-red-950 text-white border-red-800'
                      }`}>
                        <span className="text-[11px] font-bold text-slate-300 uppercase tracking-widest block mb-1">
                          Minimal Rata-Rata IPS Sisa Semester:
                        </span>
                        <div className={`text-5xl font-extrabold tracking-tight ${
                          simulationResult.isAchievable ? 'text-emerald-400' : 'text-red-400'
                        }`}>
                          {simulationResult.requiredIps <= 0
                            ? '0.00'
                            : simulationResult.requiredIps.toFixed(2)}
                        </div>
                        <div className="mt-2 text-xs font-bold">
                          {simulationResult.isAchievable ? (
                            <span className="text-emerald-300">✓ Target Realistis & Dapat Dicapai!</span>
                          ) : (
                            <span className="text-red-300">⚠️ Melebihi Skala Maksimal ({maxScale.toFixed(2)})</span>
                          )}
                        </div>
                      </div>

                      <div className="p-4 bg-slate-50 rounded-xl border border-slate-200 text-xs text-slate-700 leading-relaxed">
                        <b>Kesimpulan Analisis:</b> {simulationResult.message}
                      </div>

                      <div className="grid grid-cols-3 gap-2 text-center text-xs">
                        <div className="p-2.5 bg-slate-50 border border-slate-200 rounded-xl">
                          <span className="text-[10px] text-slate-400 block">SKS Lulus</span>
                          <span className="font-bold text-slate-900">{completedSks} SKS</span>
                        </div>
                        <div className="p-2.5 bg-slate-50 border border-slate-200 rounded-xl">
                          <span className="text-[10px] text-slate-400 block">Sisa SKS</span>
                          <span className="font-bold text-slate-900">{remainingSks} SKS</span>
                        </div>
                        <div className="p-2.5 bg-slate-50 border border-slate-200 rounded-xl">
                          <span className="text-[10px] text-slate-400 block">Total SKS Lulus</span>
                          <span className="font-bold text-slate-900">{simulationResult.totalSksAccum} SKS</span>
                        </div>
                      </div>
                    </div>
                  ) : (
                    <div className="p-8 text-center text-xs text-slate-500">
                      Masukkan parameter IPK saat ini dan sisa SKS di formulir sebelah kiri untuk melihat hasil simulasi.
                    </div>
                  )}
                </div>
