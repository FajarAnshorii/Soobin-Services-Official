  ...
  return (
    <section id="joki-skripsi" className="bg-gradient-to-br from-gray-50 to-gray-100 section-padding">
      <div className="container-custom">
        ...
        >
          <span className="inline-block bg-primary-800/10 text-primary-800 font-semibold text-sm px-4 py-2 rounded-full mb-4">
            Skripsi & Thesis
          </span>
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-dark-800 mb-4">
            JOKI SKRIPSI
          </h2>
          <div className="flex flex-wrap justify-center gap-3">
            <span className="bg-dark-800 text-white text-sm font-medium px-4 py-2 rounded-full">
              Semua Jurusan
            </span>
            <span className="bg-green-500 text-white text-sm font-medium px-4 py-2 rounded-full">
              Harga Termurah
            </span>
            <span className="bg-primary-800 text-white text-sm font-medium px-4 py-2 rounded-full">
              Terima Jadi
            </span>
          </div>
        </motion.div>
        ...
        >
          {categories.map((cat) => (
            <button
              ...
              className={`flex items-center gap-2 px-5 py-2.5 rounded-full text-sm font-medium transition-all duration-300 border-2 ${
                activeTab === cat.id
        ...
        <motion.div
          key={activeTab}
          className="bg-white rounded-3xl border border-gray-200 overflow-hidden mb-12"
          initial={{ opacity: 0, y: 20 }}
        ...
        >
          ...
          <div className="divide-y divide-gray-100">
            {visibleServices.map((service, index) => (
              <motion.div
                key={service.num}
                className="grid grid-cols-[50px_1fr] md:grid-cols-[60px_1fr_120px] gap-2 md:gap-4 px-6 py-4 hover:bg-gray-50 transition-colors duration-200 items-center"
              ...
              >
                ...
                <div className="w-10 h-10 bg-primary-800/10 rounded-lg flex items-center justify-center">
                  <span className="text-primary-800 font-bold text-sm">{service.num}</span>
                </div>

                {/* Title */}
                <div className="flex flex-col">
                  <span className="font-semibold text-dark-800 text-sm md:text-base">
                    {service.title}
                  </span>
                  <span className="md:hidden text-primary-800 font-bold text-sm mt-1">
                    Rp {service.price}
                  </span>
                </div>
            ...
            <div className="flex items-center justify-between px-6 py-4 border-t border-gray-100 bg-gray-50">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="w-8 h-8 flex items-center justify-center rounded-full border border-gray-300 text-gray-500 hover:border-primary-800 hover:text-primary-800 disabled:opacity-40 disabled:cursor-not-allowed transition-all duration-200"
                >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
          {paketCards.map((card, index) => (
            <motion.div
              key={card.title}
              className={`relative bg-white rounded-2xl p-8 text-center border-2 ${
                card.badge
            ...
            >
              ...
              <div className="bg-primary-800/10 w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-6">
                <card.icon className="w-8 h-8 text-primary-800" />
              </div>
              ...
              <div className="mt-4 pt-4 border-t border-gray-100">
                <span className="text-primary-800 font-bold">Chat Admin</span>
                <span className="text-gray-400 text-sm"> untuk harga</span>
              </div>
            </motion.div>
          ))}
        </div>
        ...
        {/* Features */}
        <motion.div
          className="bg-dark-800 rounded-3xl p-8 md:p-12"
          initial={{ opacity: 0 }}
        ...
        >
          <div className="grid md:grid-cols-3 gap-8 text-center">
            <div>
              <p className="text-4xl font-black text-primary-400 mb-2">300+</p>
              <p className="text-gray-400">Skripsi Selesai</p>
            </div>
            <div>
              <p className="text-4xl font-black text-primary-400 mb-2">50+</p>
              <p className="text-gray-400">Jurusan Ditangani</p>
            </div>
            <div>
              <p className="text-4xl font-black text-primary-400 mb-2">24/7</p>
              <p className="text-gray-400">Support Konsultasi</p>
            </div>
          </div>
        </motion.div>
        ...
        >
          <motion.a
            ...
            className="inline-flex items-center gap-2 bg-primary-800 text-white font-bold py-4 px-8 rounded-xl shadow-lg shadow-primary-800/30"
            whileHover={{
              scale: 1.02,
